...
public static class TransferEndpoints
{
    public static void MapTransferEndpoints(this WebApplication app, ApiVersionSet versionSet)
    {
        ...
        group.MapGet("/outgoing/{accountId:guid}/total", GetTotalOutgoingByAccountAsync)
            .WithName("GetTotalOutgoingByAccount")
            .Produces<decimal>(200);

        group.MapGet("/incoming/{accountId:guid}/total", GetTotalIncomingByAccountAsync)
            .WithName("GetTotalIncomingByAccount")
            .Produces<decimal>(200);
    ...
    {
        var transfers = await repo.GetAllAsync(ct);
    ...
    public static async Task<IResult> GetTotalOutgoingByAccountAsync(Guid accountId, ITransferRepository repo, CancellationToken ct)
    {
        var total = await repo.GetTotalOutgoingByAccountAsync(accountId, ct);
        return Results.Ok(total);
    }

    public static async Task<IResult> GetTotalIncomingByAccountAsync(Guid accountId, ITransferRepository repo, CancellationToken ct)
    {
        var total = await repo.GetTotalIncomingByAccountAsync(accountId, ct);
        return Results.Ok(total);
    }
    ...
    public static async Task<IResult> CreateAsync(Transfer transfer, ITransferRepository repo, CancellationToken ct)
    {
        ...
        return Results.Created($"/api/v1/transfers/{transfer.Id}", transfer);
    }
